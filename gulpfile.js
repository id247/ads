...
gulp.task('frames', function(callback){

	var folder =  devMode == 'dev' ? 'local' : 'dnevnik';
	var newDestFolder = destFolder + '/assets/frames';
...
gulp.task('vers', function(){	
	...
	const plugins = [
		...
		function frameVers(tree) {
			tree.match({ tag: 'iframe' }, function (node) {
				return setVestion(node, 'src');
			})
		},
	];
	...
	return gulp.src([destFolder + '/{dnevnik,mosreg}/**/*.html', destFolder + '/**/*.html'])
...
gulp.task('build', gulp.series('assets', 'sass', gulp.parallel('html', 'frames', 'webpack')));
...
gulp.task('prod-html', gulp.series('html', 'frames', 'vers'));
